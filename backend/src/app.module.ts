 ...
 * - CommonModule: 認証・認可の DI 対象（JwtTokenService / 各ガード）を提供する
 * - HttpExceptionFilter: APP_FILTER として登録し、全例外を統一エラー応答へ変換する
 * - AuthModule: POST /auth/register・/auth/login・/auth/logout・GET /auth/me を提供する
 * - UsersModule: メール／id によるユーザー解決・作成・プロフィール解決を提供する
